...
# 配置参数
SAVE_PATH = os.path.join(OUTPUT_DIR, "fig/fig3/compare")
...
try:
    ...
    if 'DEgene_selected' in globals():
        matrix = DEgene_selected(matrix, deg_path=PPI_DIR)
...
try:
    ...
    formatted_df.to_csv(os.path.join(OUTPUT_DIR, "fig/fig4/formatted_results.csv"), index=True)
    save_fig_path = os.path.join(OUTPUT_DIR, "fig/fig4/")
    ...
    save_dir_cm = os.path.join(OUTPUT_DIR, "fig/fig4/cm_batch")
...
try:
    ...
    save_dir_fig4 = os.path.join(OUTPUT_DIR, "fig/fig4/")
...
# ==============================================================================
# %% Block 7: Final Evaluation on Fixed 2-Gene or 1-Gene Combinations
# ==============================================================================
try:
    ACAT2 = ["ACAT2"]
    PGC = ["PGC"]
    ...
    test_aucs = plot_roc_curves(predictions, save_path=os.path.join(OUTPUT_DIR, "fig/S3fig/new/"), filename="new", figsize=(5, 5))
    plot_prf_metrics(df_results, gene_names, class_label=0, dataset_type='test',
                     save_path=os.path.join(OUTPUT_DIR, "fig/S3fig/new/"), filename="new", figsize=(9.5, 4))
    plot_prf_metrics(df_results, gene_names, class_label=1, dataset_type='test',
                     save_path=os.path.join(OUTPUT_DIR, "fig/S3fig/new/"), filename="new", figsize=(9.5, 4))
    plot_accuracy(df_results, gene_names, dataset_type='test',
                  save_path=os.path.join(OUTPUT_DIR, "fig/S3fig/new/"), filename="new", figsize=(12, 4))

    save_dir_cm = os.path.join(OUTPUT_DIR, "fig/S3fig/new/cm_batch")
    acc_summary = batch_evaluate_confusion_matrices(
        matrix_train=matrix_train, label_train=label_train,
        matrix_test=matrix_test, label_test=label_test,
        gene_combinations=gene_comb, gene_names=gene_names,
        class_labels=['HPV_CA', 'GAS'],
        save_path=save_dir_cm
    )
except Exception as e:
...
# ==============================================================================
# %% Block 8: External Validation Evaluation
# ==============================================================================
try:
    ...
    formatted_df.to_csv(os.path.join(OUTPUT_DIR, "fig/fig4/formatted_results_external.csv"), index=True)
    save_fig_path = os.path.join(OUTPUT_DIR, "fig/fig3/external/")
    os.makedirs(save_fig_path, exist_ok=True)
